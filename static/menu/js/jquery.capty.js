...
;(function($) {
	...
	$.fn.capty = function(settings) {
		...
		if (options.animation == 'slide') {
			$wrapper.hover(
				function() {
					$caption.animate({ bottom: (-1 * options.height) }, { duration: options.speed, queue: false });
				},
	    		function() {
					$caption.animate({ bottom: 0 }, { duration: options.speed, queue: false });
				}
	    	);
		} else if (options.animation == 'fade') {
			$caption.css({
				opacity:	0,
				bottom:		(-1 * options.height) + 'px'
			});
			...
			$caption.css('bottom', (-1 * options.height) + 'px');
		} else {
